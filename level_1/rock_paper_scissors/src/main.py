...
class RockPaperScissors:
    ...
    def get_item_from_user(self, user_choice):
        if user_choice.lower() in self.game_items:
            return user_choice.lower()
        else:
            print(f"please Enter from {self.game_items}")
            user_choice = input(f"Enter your choice ({self.game_items}): ")
            return self.get_item_from_user(user_choice=user_choice)
